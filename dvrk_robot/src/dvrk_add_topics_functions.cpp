                          ...
                          bool bTimestamp /* = false */)
{
    ...
    if (bTimestamp)
    {
        bridge.AddPublisherFromReadCommand<prmPositionCartesianGet, geometry_msgs::PoseStamped>
    ...
    if (bTimestamp)
    {
        bridge.AddSubscriberToWriteCommand<prmPositionCartesianSet, geometry_msgs::PoseStamped>
...
void dvrk::add_topics_mtm_timestamp(mtsROSBridge & bridge,
                          const std::string & ros_namespace,
                          const std::string & mtm_component_name)
{
    // arm API
    dvrk::add_topics_arm(bridge, ros_namespace, mtm_component_name, true);
...
{
    // arm API
    dvrk::add_topics_arm(bridge, ros_namespace, psm_component_name, true);
...
void dvrk::add_topics_ecm_timestamp(mtsROSBridge & bridge,
                          const std::string & ros_namespace,
                          const std::string & ecm_component_name)
{
    // arm API
    dvrk::add_topics_arm(bridge, ros_namespace, ecm_component_name, true);
